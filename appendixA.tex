...
$V^*$ in the rules denotes a value with locations.
\begin{figure}[ht]
  \begin{mathpar}
    \inferrule*[right=$\beta$-Lam]
    {}
    {[\sigma,(\lambda x. M^*)\ V^*] \longrightarrow [\sigma,M^*[V^*/x]]}
    \and
    \inferrule*[right=$\beta$-Pair]
    {}
    {[\sigma,\texttt{let}\ \langle x,y\rangle=\langle V^*,W^*\rangle\ \texttt{in}\ M^*] \longrightarrow [\sigma,M^*[V^*/x,W^*/y]]}
    \and
    \inferrule*[right=$\beta$-Inj$_0$]
    {}
    {[\sigma,\texttt{match}\ \texttt{inj}_0\ V^*\ [x\Rightarrow M^*\mid y\Rightarrow N^*]] \longrightarrow [\sigma,M^*[V^*/x]]}
    \and
    \inferrule*[right=$\beta$-Inj$_1$]
    {}
    {[\sigma,\texttt{match}\ \texttt{inj}_1\ V^*\ [x\Rightarrow M^*\mid y\Rightarrow N^*]] \longrightarrow [\sigma,N^*[V^*/y]]}
  \end{mathpar}
