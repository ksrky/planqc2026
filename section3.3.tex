...
Since a single reduction step can yield a distribution of configurations, we lift the reduction relation to distributions over configurations.
\begin{dfn}[Distributive reduction]
  $\mu \Longrightarrow \mu'$ iff for all $[\sigma,M]\in\mathrm{supp}(\mu)$, there exists $\nu_{\sigma,M}$ such that $[\sigma,M] \longrightarrow \nu_{\sigma,M}$ and $\mu' = \sum_{[\sigma,M]\in\mathrm{supp}(\mu)} \mu([\sigma,M])\nu_{\sigma,M}$.
